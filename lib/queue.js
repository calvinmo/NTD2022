...
export default class Queue {
  ...
  async readUntilAnyOf(routingKeys) {
    ...
    const consumer = await this.channel.consume(this.name, (message) => {
      if (message) {
        ...
        if (routingKeys.includes(message.fields.routingKey)) {
          ...
          console.log('Discarding message:', JSON.parse(message.content.toString()))
        }
      }
    }, {noAck: true, exclusive: true});

    await promiseWhile(() => polling, () => new Promise(timer => setTimeout(timer, 25)));

    await this.channel.cancel(consumer.consumerTag);
    return JSON.parse(messageOfInterest.content.toString());
  };
}
